...
BASEDIR <- "C:/Users/laura.timm/Desktop/Clupea_pallasii120/mt_genome/" #the directory where the metadata file lives
FIG_DIR <- "C:/Users/laura.timm/Desktop/Clupea_pallasii120/FIGURES/"
MT_PREFIX <- "CPAL-MTFULL" #prefix for mt genome data
DEPTH_LOWER_BOUND <- 1

CHROMFILE <- "C:/Users/laura.timm/Desktop/Clupea_pallasii120/CHAR_chrom_metadata.txt"
chrom_df <- read.table(CHROMFILE, header = TRUE)

full_metadata <- read_xlsx("C:/Users/laura.timm/Desktop/Clupea_pallasii120/pacific-herring_metadata_120.xlsx", sheet = "Sheet1")
...
## Diversity
```{r haplotype_nucleotide_div}

COR_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-COR_wgph_PRUNED.fasta'), format="fasta")
COR_hap_div <- hap.div(COR_fasta, variance = T)
COR_nuc_div <- nuc.div(COR_fasta, variance = T)

KIU_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-KIU_wgph_PRUNED.fasta'), format="fasta")
KIU_hap_div <- hap.div(KIU_fasta, variance = T)
KIU_nuc_div <- nuc.div(KIU_fasta, variance = T)

KOK_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-KOK_wgph_PRUNED.fasta'), format="fasta")
KOK_hap_div <- hap.div(KOK_fasta, variance = T)
KOK_nuc_div <- nuc.div(KOK_fasta, variance = T)

PI_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-PI_wgph_PRUNED.fasta'), format="fasta")
PI_hap_div <- hap.div(PI_fasta, variance = T)
PI_nuc_div <- nuc.div(PI_fasta, variance = T)

PM_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-PM_wgph_PRUNED.fasta'), format="fasta")
PM_hap_div <- hap.div(PM_fasta, variance = T)
PM_nuc_div <- nuc.div(PM_fasta, variance = T)

CB_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-CB_wgph_PRUNED.fasta'), format="fasta")
CB_hap_div <- hap.div(CB_fasta, variance = T)
CB_nuc_div <- nuc.div(CB_fasta, variance = T)

TOG_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-TOG_wgph_PRUNED.fasta'), format="fasta")
TOG_hap_div <- hap.div(TOG_fasta, variance = T)
TOG_nuc_div <- nuc.div(TOG_fasta, variance = T)

ebs_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-eBS_wgph_PRUNED.fasta'), format="fasta")
ebs_hap_div <- hap.div(ebs_fasta, variance = T)
ebs_nuc_div <- nuc.div(ebs_fasta, variance = T)

goa_fasta <- read.dna(paste0(BASEDIR, MT_PREFIX, '-GOA_wgph_PRUNED.fasta'), format="fasta")
goa_hap_div <- hap.div(goa_fasta, variance = T)
goa_nuc_div <- nuc.div(goa_fasta, variance = T)

goa1_fasta <- read.dna(paste0(BASEDIR, 'GOA1.fasta'), format="fasta")
goa1_hap_div <- hap.div(goa1_fasta, variance = T)
goa1_nuc_div <- nuc.div(goa1_fasta, variance = T)

goa2_fasta <- read.dna(paste0(BASEDIR, 'GOA2.fasta'), format="fasta")
goa2_hap_div <- hap.div(goa2_fasta, variance = T)
goa2_nuc_div <- nuc.div(goa2_fasta, variance = T)
